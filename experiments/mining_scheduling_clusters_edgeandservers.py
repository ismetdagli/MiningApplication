...
argument_edge_group = int(sys.argv[5])
argument_server_group = int(sys.argv[6])



number_of_edges=int(argument_edges)
number_of_servers=int(argument_servers)
edge_group=int(argument_edge_group) 
server_group=int(argument_server_group) 
...
def find_my_server_group(edge_id):
    #edge groups: 0 1 2for server 0 
    #             2 3 for server 1

    #edge groups: 0 1 2 for server 0 
    #             3 4 5 for server 1
    if (edge_id/edge_group)<1:
        return 0
    else:
        return 1

def find_my_edge_group(edge_id):
    #edge groups: 0 1 2for server 0 
    #             2 3 for server 1
    ...
    # if edge_id/edge_group 

same_group_found=[0,0] # 0 for yes, 1 for no
...
while True:
    if (sim_time < 5000):
        ...
        if len(queue)>0:
            ...
            #checking servers
            if PU == None:  
                print("No available edges found, search servers available")
                if current_task[0] == "svm":
                ...
                for server_id in range(number_of_edges, number_of_edges+number_of_servers):
                    print("asking to same server group :", current_task, "served id: ",server_id)
                ...
                same_group_available = False

                for server_ids in available_server_ids:
                    print("server id,", server_ids, " find_my_edge_group(current_task[1]): ", find_my_edge_group(current_task[1]) ,
                          " find_my_server_group(server_ids) ",find_my_server_group(server_ids))
                    if find_my_edge_group(current_task[1]) == find_my_server_group(server_ids):
                        index=server_ids
                        same_group_available=True
                        same_group_found[0]+=1
                        break
        ...
        if (len(queue) > 0):
            ...
            if last_edge_read_data>(number_of_edges-1):
                last_edge_read_data=0
            queue.append(["svm",last_edge_read_data])
            queue.append(["knn",last_edge_read_data])
            queue.append(["mlp",last_edge_read_data])
        ...
        print("-------------------------------------------------------")
        iteration+=1
        if ((task_completed > 10*number_of_edges) or (100 < sim_time)):
            ...
            print("same_group_found: ",same_group_found)
