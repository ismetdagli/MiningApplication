...
queue = []
for i in range(int(tasks_amount)):
    queue.append(["mlp",i])
    queue.append(["knn",i])
    queue.append(["svm",i])
...
def find_task_completed_sim_time(working_task,next_frame_time):
    ...
    else: # next sensor data will start
        return "next sensor data",next_frame_time,index
...
while True:
    if (sim_time < 5000):
        ...
        task,time_estimated,index=find_task_completed_sim_time(working_task,FPS*(frame_number+1))
        ...
        if (len(queue) > 0):
        ...
        elif task == "next sensor data":
            print("next sensor data will be called")
            frame_number+=1
            ...
            if len(qos_failed_tasks) > 0 :
                next_task_process=qos_failed_tasks.pop()
                queue.append(next_task_process)
            # qos_failed_tasks=[]
        #print("-------------------------------------------------------")
        iteration+=1
        if ((task_completed > 9998) ):
