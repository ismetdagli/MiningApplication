...
argument_onlyserver = int(sys.argv[4])
...
queue = []
last_edge_read_data=0
queue.append(["svm",last_edge_read_data])
queue.append(["knn",last_edge_read_data])
queue.append(["mlp",last_edge_read_data])
# for i in range(number_of_edges):
#     queue.append(["svm",i])
#     queue.append(["knn",i])
#     queue.append(["mlp",i])
...
def update_multi_tenancy_after_assignment():
    ...
    for i in range(number_of_servers):
        tasks_in_servers_gpu.append(0)
    for i in range(len(working_task)):
        #print("task device id: ", working_task[i][3])
...
tasks_assigned_to_server=[]

for i in range(number_of_edges):
    tasks_assigned_to_server.append([-1,-1,-1])

task_assign_same_server=[0,0,0]
task_assign_diff_server=[0,0,0]
    ...
    if (sim_time < 5000):
        ...
        if len(queue)>0:
            ...
            if PU != None: #we assigned task to one PU
                ...
                working_task.append([current_task[0],sim_time+shortest_time,PU,current_task[1],updated,frame_number,sim_time]) # 0 is edge
                check_device_availability()
            # #Checking other edges in the network
            if argument_onlyserver == 0:
                if PU == None:
                    if current_task[0] == "svm":
                        tasks_asked_edge[0]+=1
                    if current_task[0] == "knn":
                        tasks_asked_edge[1]+=1
                    if current_task[0] == "mlp":
                        tasks_asked_edge[2]+=1
                    tasks_failed_to_assign_edge[current_task[1]]+=1 #first edge device could not assign the task
                    ...
                    for i in range (0,number_of_edges):
                        if current_task[1]==i:
                            shortest_times_edges.append([None,None])
                            continue
                    ...
                    shortest_time=999
                    index=-1
                    #print("shortest time edges: ",shortest_times_edges)
                    ...
                    if index != -1:
                        assign_task_to_PU(index,0,shortest_times_edges[index][0])   #edge/device index, device number, PU
                        #print("current_task[0] and PU: ", current_task[0],shortest_times_edges[index][0])
                        memory_use[index]+=bandwidth(current_task[0],shortest_times_edges[index][0])
                        updated=update_expected_finish_time_edge(index)
                        working_task.append([current_task[0],sim_time+shortest_time,shortest_times_edges[index][0],index,updated,current_task[1],sim_time]) # 0 is edge
                        #print("Edge id: ", index+1," assigned task:", current_task)
                        updated=update_expected_finish_time_edge(index)       
                        if sim_time_reminder_server!=sim_time:
                            bandwidth_when_ask_edge+=1.0
                            sim_time_reminder_server=sim_time
                    else: 
                        PU=None
                    check_device_availability()
            ...
            if PU == None:  
                ...
                assign_task_to_other_server=True
                server_index=tasks_assigned_to_server[current_task[1]][task_index(current_task[0])]
                if server_index != -1:
                    #print("tasks assigned to server ",server_index)
                    PU, shortest_time=attempt_run_on_server(current_task[0],deadline_per_task[task_index(current_task[0])],server_index+number_of_edges)
                    # #print()
                    if PU != None:
                        assign_task_to_PU(server_index+number_of_edges,0,PU) 
                        assign_task_to_other_server=False
                        #print("Served assigned same server:", current_task)
                        #print ("index: ",server_index, "  allow_server_multi_tenancy: ", allow_server_multi_tenancy, " gpu_available: ",gpu_available)
                        ...
                        updated=update_expected_finish_time_server(server_index+number_of_edges)
                        
                        working_task.append([current_task[0],shortest_time+sim_time,PU,server_index+number_of_edges,updated,current_task[1],sim_time]) 
                        #print("working task same server assignment", working_task)
                        update_multi_tenancy_after_assignment()
                        task_assign_same_server[task_index(current_task[0])]+=1
                if assign_task_to_other_server:
                    PUs=[]
                    shortest_times_servers=[]
                    for server_id in range(number_of_edges, number_of_edges+number_of_servers):
                        #print("asking to other Server :", current_task, "served id: ",server_id)
                    ...
                    if not(available_PU_found(PUs)):
                        ...
                        tasks_assigned_to_server[current_task[1]][task_index(current_task[0])]=index
                        #print("Served assigned different task:", current_task)
                        #print("tasks_assigned_to_server updated:", tasks_assigned_to_server)
                        ...
                        if sim_time_reminder_server!=sim_time:
                            ...
                            sim_time_reminder_server=sim_time
                        working_task.append([current_task[0],shortest_times_servers[index]+sim_time,PUs[index],index+number_of_edges,updated,current_task[1],sim_time]) 
                        update_multi_tenancy_after_assignment()
                        task_assign_diff_server[task_index(current_task[0])]+=1
        ...
        if (len(queue) > 0):
            ...
            last_edge_read_data+=1
            if last_edge_read_data>4:
                last_edge_read_data=0
            queue.append(["svm",last_edge_read_data])
            queue.append(["knn",last_edge_read_data])
            queue.append(["mlp",last_edge_read_data])
            ...
            task_processing_time.append([task[1],task[6],task[0]])
        ...
        if ((task_completed > 100*number_of_edges) or (1000 < sim_time)):
            ...
            print("bandwidth_when_ask_edge: ",bandwidth_when_ask_edge)
            print("Overhead with bandwidth_when_ask_edge: %",100*bandwidth_when_ask_edge/sim_time/number_of_edges)
            ...
            print("tasks_asked_edge: ",tasks_asked_edge)
            print("tasks_asked_server: ",tasks_asked_server)
            print("task_assign_same_server: ",task_assign_same_server)
            print("task_assign_diff_server: ",task_assign_diff_server)
            ...
            completion_time_per_tasks_total=0
            completion_time_per_task0=0
            completion_time_per_task1=0
            completion_time_per_task2=0
            task_completed0=0
            task_completed1=0
            task_completed2=0
            for i in range((len(task_processing_time))):
                completion_time_per_tasks_total+=task_processing_time[i][0]-task_processing_time[i][1]
                if task_processing_time[i][2]=="svm":
                    completion_time_per_task0+=task_processing_time[i][0]-task_processing_time[i][1]
                    task_completed0+=1
                elif task_processing_time[i][2]=="knn":
                    completion_time_per_task1+=task_processing_time[i][0]-task_processing_time[i][1]
                    task_completed1+=1
                elif task_processing_time[i][2]=="mlp":
                    completion_time_per_task2+=task_processing_time[i][0]-task_processing_time[i][1]
                    task_completed2+=1
            print("task_processing_time_total: ",completion_time_per_tasks_total/task_completed)
            print("task_processing_time0: ",completion_time_per_task0/task_completed0)
            print("task_processing_time1: ",completion_time_per_task1/task_completed1)
            print("task_processing_time2: ",completion_time_per_task2/task_completed2)
            #print("processing total times",task_processing_time)
